...
public class RestDiscoverQueryBuilder {
     ...
     * @param discoveryConfiguration the discovery configuration for this discovery
     *                               query
     ...
     * Build a discovery query.
     *
     * @param searchType optional search type (e.g. semantic)
     */
    public DiscoverQuery buildQuery(Context context, IndexableObject<?, ?> scope,
            DiscoveryConfiguration discoveryConfiguration,
            String query, List<SearchFilter> searchFilters,
            List<String> dsoTypes, Pageable page, String searchType)
            throws DSpaceBadRequestException {

        DiscoverQuery discoverQuery = buildQuery(context, scope, discoveryConfiguration, query, searchFilters,
                dsoTypes, page);
        applySearchType(discoverQuery, searchType);
     ...
     * @param query                  the query string for this discovery query
     * @param searchFilters          the search filters for this discovery query
     * @param dsoTypes               only include search results with one of these
     *                               types
     * @param page                   the pageable for this discovery query
     */
    public DiscoverQuery buildQuery(Context context, IndexableObject<?, ?> scope,
     ...
     * @param discoveryConfiguration the discovery configuration for this discovery
     *                               query
     * @param prefix                 limit the facets results to those starting with
     *                               the given prefix.
     * @param query                  the query string for this discovery query
     * @param searchFilters          the search filters for this discovery query
     * @param dsoType                only include search results with this type
    ...
    public DiscoverQuery buildFacetQuery(Context context, IndexableObject<?, ?> scope,
        ...
        DiscoverQuery discoverQuery = buildFacetQuery(context, scope, discoveryConfiguration, prefix, query,
    ...
    private void applySearchType(DiscoverQuery discoverQuery, String searchType) {
        if (discoverQuery != null && StringUtils.isNotBlank(searchType)) {
            discoverQuery.addProperty("searchType", searchType);
        }
    }
